...
let
  ...
  configPath =
    "${(if pkgs.stdenv.isDarwin then darwinConfigPath else linuxConfigPath)}";
  profilesPath =
    if pkgs.stdenv.isDarwin then "${configPath}/Profiles" else configPath;

  mkFirefoxModule = import
    "${home-manager.outPath}/modules/programs/firefox/mkFirefoxModule.nix";
in {
  imports = [
    (mkFirefoxModule {
      inherit modulePath;
        ...
        attrsOf (submodule ({ ... }: {
          options = {
            spacesForce = mkOption {
              type = bool;
              description =
                "Whether to delete existing spaces not declared in the configuration.";
              default = false;
            };
            spaces = mkOption {
                  ...
                  theme.colors = mkOption {
                    type = nullOr (listOf (submodule ({ ... }: {
                      options = {
                        ...
                        algorithm = mkOption {
                          type =
                            enum [ "complementary" "floating" "analogous" ];
                          default = "floating";
                        };
                        primary = mkOption {
                          type = bool;
  ...
  config = mkIf cfg.enable {
    ...
    home.file = let
      ...
      inherit (lib)
        concatStringsSep concatMapStringsSep concatMapAttrsStringSep filterAttrs
        getExe getExe' mapAttrs' mapAttrsToList nameValuePair optionalString
        pipe;
    in (mapAttrs' (profileName: profile:
      let
        sqlite3 = getExe' pkgs.sqlite "sqlite3";
        scriptFile = "${configPath}/${profileName}/places_update.sh";
        placesFile =
          "${config.home.homeDirectory}/${configPath}/${profileName}/places.sqlite";
          ...
          ${sqlite3} "${placesFile}" "${
            (concatStringsSep " " [
              ...
              (mapAttrsToList (_: s: [
                ...
                (if isNull s.theme.opacity then
                  "NULL"
                else
                  toString s.theme.opacity)
                (if isNull s.theme.rotation then
                  "NULL"
                else
                  toString s.theme.rotation)
                (if isNull s.theme.texture then
                  "NULL"
                else
                  toString s.theme.texture)
        ...
        deleteSpaces = ''
          ${sqlite3} "${placesFile}" "DELETE FROM zen_workspaces ${
            ...
            concatMapAttrsStringSep " AND " (_: s: "NOT uuid = '{${s.id}}'")
            profile.spaces
          }" || exit 1
        '';
      in nameValuePair scriptFile {
        source = getExe
        ...
        executable = true;
        force = true;
      }) (filterAttrs (_: profile: profile.spaces != { } || profile.spacesForce)
        cfg.profiles));
